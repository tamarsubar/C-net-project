...
internal class SaleImplementation : BL.BlApi.ISale
{
    ...
    public int Create(BL.BO.Sale sale)
    {
        ...
        catch (BL.BO.BlAlreadyExistsException ex) { throw new BL.BO.BlAlreadyExistsException("מבצע קיים", ex); }
    }

    public BL.BO.Sale? Read(int id)
    {
        try { return _dal.Sale.Read(id)?.ToBO(); }
        catch (BL.BO.BlDoesNotExistException ex) { throw new BL.BO.BlDoesNotExistException("מבצע לא נמצא", ex); }
    }
    ...
    public void Delete(int id)
    {
        ...
        catch (BL.BO.BlDoesNotExistException ex) { throw new BL.BO.BlDoesNotExistException("לא ניתן למחוק", ex); }
    }
}